...
// ── Format apparel/shop cart item for email ────────────────────────────────────
...
// ── Generate Apparel Work Order PDF using PDFKit ────────────────────────────
function generateShopWorkOrderPDF(orderId, totalAmount, subtotalAmount, taxAmount, customer, cartItems, orderNotes) {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ margin: 36, size: 'letter' });
    const buffers = [];
    doc.on('data', chunk => buffers.push(chunk));
    ...
    doc.fillColor('#9a8ab0').fontSize(8).font('Helvetica')
       .text('419 N. 1st Street, Montrose, CO 81401  |  (970) 249-4418  |  ColumbineCopy.com', 44, y + 22, { width: W - 100 });
    doc.fillColor('#9a8ab0').fontSize(7).font('Helvetica')
       .text('APPAREL ORDER', 44, y + 6, { align: 'right', width: W - 8 });
    doc.fillColor('#c8a0f0').fontSize(11).font('Helvetica-Bold')
       .text(orderId, 44, y + 17, { align: 'right', width: W - 8 });
    y += 42;
    ...
    // ── Order Items (single column — apparel items carry more detail per row) ──
    const items = cartItems || [];
    items.forEach((item, index) => {
      ...
      const lines = [
        ['Garment', `${item.productId || ''} — ${item.productName || ''}${item.brand ? ` (${item.brand})` : ''}`],
        ['Color', item.color || '—'],
        ['Front', sides.front && sides.front.hasArtwork && !sides.front.noPrint
          ? `${methodLabel(sides.front.method)} — ${sizeLabel(sides.front.size)}${sides.front.artBad ? '  [LOW-RES — ART FEE]' : ''}`
          : 'No print'],
        ['Back', sides.back && sides.back.hasArtwork && !sides.back.noPrint
          ? `${methodLabel(sides.back.method)} — ${sizeLabel(sides.back.size)}${sides.back.artBad ? '  [LOW-RES — ART FEE]' : ''}`
          : 'No print'],
        ['Quantity', `${qtyStr}  (${item.totalQty || 0} total)`],
        item.upchargeTotal > 0 ? ['Size Upcharges', `$${Number(item.upchargeTotal).toFixed(2)}`] : null,
        ['Front Artwork', item.frontUrl || 'none uploaded'],
        ['Back Artwork', item.backUrl || 'none uploaded'],
        item.notes ? ['Notes', item.notes] : null,
        ['Item Total', `$${Number(item.itemTotal || 0).toFixed(2)}`],
      ].filter(Boolean);

      // Estimate row heights (long URLs / notes may wrap)
      let rowsH = 14;
      lines.forEach(([label, value]) => {
        const wrap = ['Notes', 'Front Artwork', 'Back Artwork'].includes(label);
        rowsH += wrap ? Math.max(11, doc.heightOfString(String(value), { width: W - 130, fontSize: 7.5 }) + 6) : 11;
      });
      rowsH += 4;

      if (y + rowsH > doc.page.height - 80) {
        doc.addPage();
        y = 36;
      }

      doc.rect(36, y, W, 13).fill('#1a0a2e');
      doc.fillColor('#fff').fontSize(8).font('Helvetica-Bold')
         .text(`Item ${index + 1}`, 40, y + 3, { width: W - 8 });
      let iy = y + 14;
      lines.forEach(([label, value], li) => {
        const wrap = ['Notes', 'Front Artwork', 'Back Artwork'].includes(label);
        const strVal = String(value);
        let rowH = 11;
        if (wrap) rowH = Math.max(11, doc.heightOfString(strVal, { width: W - 130, fontSize: 7.5 }) + 6);
        doc.rect(36, iy, W, rowH).fill(li % 2 === 0 ? '#f4f0fb' : '#fff');
        doc.fillColor('#666').fontSize(7.5).font('Helvetica')
           .text(label + ':', 40, iy + 2, { width: 90 });
        doc.fillColor('#1a0a2e').font('Helvetica-Bold')
           .text(strVal, 132, iy + 2, { width: W - 100 });
        iy += rowH;
      });
      doc.rect(36, y, W, iy - y).stroke('#d4c8e8');
      y = iy + 8;
    });
    ...
    doc.fillColor('#555').fontSize(7.5).font('Helvetica-Bold')
       .text('STATUS:', 42, y + 5);
    const statuses = ['Art Reviewed', 'In Production', 'Ready for Pickup', 'Shipped'];
    let sx = 100;
    statuses.forEach(s => {
      doc.rect(sx, y + 4, 9, 9).stroke('#333');
      doc.fillColor('#333').font('Helvetica').text(s, sx + 12, y + 5);
    ...
    doc.fillColor('#999').fontSize(7).font('Helvetica')
       .text('Columbine Copy & Apparel  ·  419 N. 1st Street, Montrose, CO 81401  ·  (970) 249-4418  ·  ColumbineCopy.com', 36, y, { align: 'center', width: W })
       .text('Order reviewed before production per Order Terms & Agreement accepted at checkout', 36, y + 9, { align: 'center', width: W });
...
exports.handler = async function(event) {
  ...
  const { sourceId, amountCents, currency, customer, cartItems, orderNotes, pdfFiles, isShopOrder, termsAccepted, termsAcceptedAt } = body;
  ...
  try {
    const response = await squareClient.paymentsApi.createPayment({
      ...
      referenceId: orderId,
      note: `Columbine ${isShopOrder ? 'Apparel' : 'Print'} Order ${orderId} — ${customer?.name || 'Customer'}`,
  ...
  try {
    const pdfBuffer = isShopOrder
      ? await generateShopWorkOrderPDF(orderId, totalAmount, subtotalAmount, taxAmount, customer, cartItems, orderNotes)
      : await generateWorkOrderPDF(orderId, totalAmount, subtotalAmount, taxAmount, customer, cartItems, orderNotes);
  ...
  // Build cart HTML — print jobs use a 2-column grid; apparel items use a
  // single-column list since each item carries more detail (sides, qty
  // breakdown, artwork links).
  const items = cartItems || [];
  const formatItemFn = isShopOrder ? formatShopCartItem : formatCartItem;

  let cartHtml, page1Items, remainingItems;

  if (isShopOrder) {
    cartHtml = items.map((item, i) => formatItemFn(item, i)).join('');
  } else {
    const buildGrid = (itemsSlice, startIndex) => {
      const rows = [];
      for (let i = 0; i < itemsSlice.length; i += 2) {
        const left = itemsSlice[i] ? formatItemFn(itemsSlice[i], startIndex + i) : '';
        const right = itemsSlice[i+1] ? formatItemFn(itemsSlice[i+1], startIndex + i + 1) : '<div></div>';
    ...
    page1Items = items.slice(0, 4);
    remainingItems = items.slice(4);

    cartHtml = buildGrid(page1Items, 0);
  ...
  await sendEmail(
    ...
    isShopOrder
      ? `🛒 New Apparel Order ${orderId} — $${totalAmount} — ${customer?.name}`
    ...
    `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
      <div style="background:#1a0a2e;padding:24px;border-radius:6px 6px 0 0">
        <h1 style="color:#c8a0f0;font-size:1.4rem;margin:0">${isShopOrder ? 'New Apparel Order' : 'New Print Order'}</h1>
      ...
      <div style="background:#fff;padding:24px;border:1px solid #d4c8e8;border-top:none">
        ...
        <div style="background:#f4f0fb;border-radius:6px;padding:16px 20px;margin-bottom:20px;font-size:.95rem">
          ...
          ${isShopOrder ? `<div style="margin-top:8px;font-size:.82rem;color:${termsAccepted ? '#2e7d32' : '#b71c1c'}">${termsAccepted ? '✅' : '⚠️'} Order Terms & Agreement ${termsAccepted ? `accepted at ${termsAcceptedAt || 'checkout'}` : 'NOT confirmed accepted'}</div>` : ''}
        </div>
        ${!isShopOrder && fileLinks.length > 0 ? `
        ...
        ${!isShopOrder && attachments.length > 0 ? `
        ...
        ${!isShopOrder && fileLinks.length === 0 && attachments.length === 0 ? `
  ...
  if (customer?.email) {
    const shopItemsHtml = isShopOrder ? (cartItems || []).map(item => {
      const qtyBreakdown = item.qtyBreakdown || {};
      const qtyStr = Object.entries(qtyBreakdown).map(([size, qty]) => `${qty}×${size}`).join(', ') || '—';
    ...
    await sendEmail(
      customer.email,
      isShopOrder ? `Your apparel order is confirmed — ${orderId}` : `Your print order is confirmed — ${orderId}`,
      `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
        ...
        <div style="background:#fff;padding:24px;border:1px solid #d4c8e8;border-top:none;text-align:center">
          ...
          <p style="color:#555;font-size:.9rem">${isShopOrder
            ? 'Thank you for your order! We have received your payment and your order will be reviewed before production begins, per the Order Terms & Agreement you accepted at checkout.'
            : 'Thank you for your order! We have received your payment and will begin processing your print job shortly.'}</p>
          ...
          ${isShopOrder && shopItemsHtml ? `<div style="margin:16px 0;text-align:left"><div style="color:#6e5a8a;font-size:.78rem;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;text-align:center">Your Order</div>${shopItemsHtml}</div>` : ''}
          <p style="color:#333">Total paid: <b>$${totalAmount}</b></p>
          <p style="color:#555;font-size:.88rem">${isShopOrder
            ? 'Most orders are completed within 10 business days or less, not including shipping. We\'ll reach out if there\'s any issue with your artwork, stock availability, or order details before we begin production.'
            : 'Your order will be printed during normal business hours and you will be notified by email once it is ready for pickup — usually within 30 minutes.'}</p>
          <p style="color:#999;font-size:.78rem;margin-top:24px">Columbine Copy & Apparel · ${isShopOrder ? 'Questions about your order? Reply to this email or call (970) 249-4418.' : 'All files are kept confidential and deleted after printing'}</p>
        </div>
      </div>`
    );
  }
